...
class Simplex
{
    ...
    public function get(xin :Float, yin :Float) : Float
    {
        ...
        i &= 255;
        j &= 255;
        var gi0 = _gradP[i+_perm[j]];
        var gi1 = _gradP[i+i1+_perm[j+j1]];
        var gi2 = _gradP[i+1+_perm[j+1]];
    ...
    private function init(seed :Int) : Void
    {
        if(seed > 0 && seed < 1) {
            // Scale the seed out
            seed *= 65536;
    ...
    private static var F2 = 0.5*(Math.sqrt(3)-1);
